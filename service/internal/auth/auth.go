...
func IsSchoolOwner(db *sqlx.DB, schoolID string, userID string) (bool, error) {
	var exists bool
	err := db.Get(&exists, `SELECT EXISTS (SELECT 1 FROM schools WHERE id=$1 AND owner_user_id=$2)`, schoolID, userID)
	return exists, err
}
